...
function Add({setdisplayformstate,data}) {
  ...
  //***************************************************************************************************************/
    ...
    if(valid === true){
      ...
      const addDossier = () =>{
        Axios.post(env.hostUrl+"/api/dossier/add", DossierAdd, config).then((response) => {
          window.setTimeout(() => {
            setdisplayformstate({bool3:false,blur:1,disable:"auto"})
         }, 2000)
          }).catch(error => {
    ...
    <div id='add-doss-body' data-theme={data}>
        ...
        <div className='add-form-container'>
            <div class="add-doss-title-box">
                <h1 className="add-doss-title titles" data-theme={data}>Créer un nouveau dossier!</h1>
            </div>
        ...
        <div className="btn-AjouterDoss">
            <button className="btn-ajouter-doss" onClick={Validation} >Créer</button>
          </div>
    </div>
    </div>
  )
